import Link from 'next/link'

export default function HomePage() {
  return (
    <div className="min-h-screen">
      {/* Header */}
      <header className="bg-white shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center">
              <h1 className="text-2xl font-bold text-primary-600">Vouch</h1>
            </div>
            <nav className="flex space-x-4">
              <Link href="/products" className="text-gray-600 hover:text-primary-600">
                Browse
              </Link>
              <Link href="/sell" className="text-gray-600 hover:text-primary-600">
                Sell
              </Link>
              <Link href="/login" className="btn btn-primary">
                Login
              </Link>
            </nav>
          </div>
        </div>
      </header>

      {/* Hero Section */}
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="text-center">
          <h1 className="text-4xl font-bold text-gray-900 sm:text-5xl md:text-6xl">
            Guaranteed Authentic
            <span className="text-primary-600"> Luxury Resale</span>
          </h1>
          <p className="mt-3 max-w-md mx-auto text-base text-gray-500 sm:text-lg md:mt-5 md:text-xl md:max-w-3xl">
            Buy and sell authenticated luxury items with confidence. Every item is verified by third-party experts, and your payment is protected by secure escrow.
          </p>
          <div className="mt-5 max-w-md mx-auto sm:flex sm:justify-center md:mt-8">
            <div className="rounded-md shadow">
              <Link href="/products" className="w-full flex items-center justify-center px-8 py-3 border border-transparent text-base font-medium rounded-md text-white bg-primary-600 hover:bg-primary-700 md:py-4 md:text-lg md:px-10">
                Shop Now
              </Link>
            </div>
            <div className="mt-3 rounded-md shadow sm:mt-0 sm:ml-3">
              <Link href="/sell" className="w-full flex items-center justify-center px-8 py-3 border border-transparent text-base font-medium rounded-md text-primary-600 bg-white hover:bg-gray-50 md:py-4 md:text-lg md:px-10">
                Start Selling
              </Link>
            </div>
          </div>
        </div>

        {/* Features */}
        <div className="mt-20">
          <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
            <div className="card text-center">
              <div className="text-primary-600 text-4xl mb-4">🔒</div>
              <h3 className="text-lg font-medium text-gray-900">Secure Escrow</h3>
              <p className="mt-2 text-base text-gray-500">
                Your payment is held safely until you receive and approve your item.
              </p>
            </div>
            <div className="card text-center">
              <div className="text-primary-600 text-4xl mb-4">✅</div>
              <h3 className="text-lg font-medium text-gray-900">Authentication</h3>
              <p className="mt-2 text-base text-gray-500">
                Every item is verified by professional authentication services.
              </p>
            </div>
            <div className="card text-center">
              <div className="text-primary-600 text-4xl mb-4">📦</div>
              <h3 className="text-lg font-medium text-gray-900">Safe Shipping</h3>
              <p className="mt-2 text-base text-gray-500">
                Tracked and insured shipping with trusted carriers.
              </p>
            </div>
          </div>
        </div>
      </main>
    </div>
  )
}